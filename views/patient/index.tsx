...
import PatientQcList from "./components/qcList";

const menuItems = [{ label: "诊疗记录", component: PatientQcList }];

const getPhoneFromCookie = () => {
  ...
  const cookieEntry = document.cookie
    .split(";")
    .map((item) => item.trim())
    .find((item) => item.startsWith("qc_phone="));
  if (!cookieEntry) {
    return "";
  }
...
const Patient = () => {
  ...
  const [userPhone] = useState(() => getPhoneFromCookie());
  const ActiveComponent =
    menuItems[activeMenuIndex]?.component || PatientQcList;
    ...
    <div className="min-h-screen bg-[#f5f7fb] text-[#1d1d1f] flex">
      <aside className="w-72 bg-white flex flex-col p-6 shadow-[4px_0_25px_rgba(0,0,0,0.05)]">
        <div className="flex items-center gap-4 pb-6 border-b border-[#f0f0f0]">
          ...
          <div>
            <div className="text-lg font-semibold">张 小雅</div>
            <div className="text-sm text-[#848484]">患者</div>
            {userPhone && (
              <div className="text-xs text-[#a0a0a0]">手机号 {userPhone}</div>
            )}
          </div>
        </div>
